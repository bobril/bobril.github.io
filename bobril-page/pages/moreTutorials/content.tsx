...
`}</code>
    ...
    <p>
      <a href="./static-examples/hello-with-children/index.html">{`Preview example`}</a>
    </p>
...
`}</code>
    ...
    <p>
      <a href="./static-examples/todo-advanced/index.html">{`Preview example`}</a>
    </p>
...
`}</code>
    ...
    <p>
      <a href="./static-examples/todo-advanced-bobx/index.html">{`Preview example`}</a>
    </p>
...
`}</code>
    ...
    <p>
      ...
      <code>bobril-build</code>
      {` page on `}
      <a href="http://localhost:8080/bb/">http://localhost:8080/bb/</a>
      {`. The tree can be filtered by the result and sorted by path or describe hierarchy.`}
    </p>
    <p>
      <img
        style={{ maxWidth: "100%" }}
        src="https://github.com/bobril/bobril.github.io/raw/master/resources/md-images/result-page.png"
        alt="Test and build results"
      />
    </p>
    <p>
      {`You can also check the code coverage. If you switch the `}
      <em>{`Coverage`}</em>
      {` button on then the coverage is represented as a highlighted code in the Visual Studio Code:`}
    </p>
    <p>
      <img
        style={{ maxWidth: "100%" }}
        src="https://github.com/bobril/bobril.github.io/raw/master/resources/md-images/code-coverage.png"
        alt="Code coverage"
      />
    </p>
    ...
    <ul>
      <li>
        <code>b.route</code>
        {` - defines route parameters and a list of sub-routes`}
      </li>
      <li>
        <code>b.routes</code>
        {` - registers routes to the application and calls `}
        <code>b.init</code>
      </li>
      <li>
        <code>b.routeDefault</code>
        {` - defines the default target route (the final leaf of the routing tree) if no sub-route is specified in the current url`}
      </li>
    </ul>
    ...
    <ul>
      <li>
        <code>url</code>
        {` - Define the url of the route. A part of it can also include URL parameters. With these parameters, it is possible to modify how the page will be displayed.`}
      </li>
      <li>
        <code>name</code>
        {` - Define the name of the route. The name is used as an identifier during redirection. Name cannot contain “:” or “/“.`}
      </li>
      <li>
        <code>handler</code>
        {` - Define the content of a page at a specified URL. The content is rendered after redirection. Different routes can have the same handler.`}
      </li>
      <li>
        <code>data</code>
        {` - Already defined information for a specific handler. For example, there is one handler used in different routes. In one route, I want an editable page, and in the other one, I want to a read-only page. Both of these routes will have the same handler, but different parameter readOnly.`}
      </li>
      <li>
        <code>keyBuilder</code>
...
`}</code>
    ...
    <p>
      <a href="./static-examples/routing/index.html">{`Preview example`}</a>
    </p>
    <p>
      ...
      <code>PageOne</code>
      {` will be used when no sub-route is specified.`}
    </p>
    <p>
      {`The url for page `}
      <em>{`two`}</em>
      {` contains a parameter specification after a second slash. It is defined by a `}
      <strong>{`colon`}</strong>
      {` and a `}
      <strong>{`name`}</strong>
      {` of the parameter. The `}
      <strong>{`question mark`}</strong>
      {` defines the parameter as optional. Route parameters can then be found in the input data prpoerty `}
    ...
    <p>
      {`Now we will write a `}
      <code>MainPage</code>
      {` component to render some own content and the visual content of the active sub-route. To do this a function provided in `}
      <code>data.activeRouteHandler</code>
      {` is used.`}
    </p>
...
`}</code>
    ...
    <ul>
      <li>
        <code>b.IRouteTransition</code>
        {` - interface for a transition definition (target name, parameters etc.)`}
      </li>
      <li>
        <code>b.createRedirectReplace</code>
        {` - creates IRouteTransition object for redirect without saving history`}
      </li>
      <li>
        <code>b.createRedirectPush</code>
        {` - creates IRouteTransition object for redirect with saving history`}
      </li>
      <li>
        <code>b.runTransition</code>
        {` - runs a transition according to an input IRouteTransition object`}
      </li>
      <li>
        <code>b.link</code>
        {` - changes an input IBobrilNode to a link to the route of a specified name and with specified optional params`}
      </li>
      <li>
        <code>&lt;Link&gt;</code>
        {` - b.link functiona s a `}
        <code>component</code>
      </li>
    </ul>
    <p>
      {`Example of the redirect definition from the `}
      <code>PageOne</code>
      {` to the `}
      <code>PageTwo</code>
      {`:`}
...
`}</code>
    ...
    <p>
      {`The code in onClick callback of button creates and runs a transition to the page `}
      <em>{`two`}</em>
      {` with an object defining the value of a `}
      <em>{`text`}</em>
      {` parameter.`}
    </p>
    <p>
      {`The transition above should be handled by the `}
      <code>PageTwo</code>
      {` handler.`}
    </p>
...
`}</code>
    ...
    <p>
      {`The page receives the text parameter value in its `}
      <code>data.routeParams.text</code>
      {`. It also defines a link node to the page `}
      <em>{`one`}</em>
      {` by `}
      <code>&lt;Link name=&quot;one&quot;&gt;</code>
      {` around `}
      <code>&lt;a&gt;</code>
      {` element.`}
    </p>
    ...
    <p>
      {`The example also contains codes determining the possibility to enter or leave the page. For these purposes, we can use the following static functions of `}
      <code>IBobrilComponent</code>
      {`:`}
    </p>
    <ul>
      <li>
        <code>static canActivate</code>
    ...
    <p>
      {`The previous code also contains example, which handles leaving the page `}
      <em>{`one`}</em>
      {` with empty value of a textbox by adding `}
      <em>{`canDeactivate`}</em>
      {` function and example of handling the not logged user on accessing the page `}
      <em>{`two`}</em>
      {` by adding `}
      <code>static canActivate</code>
      {` function definition.`}
    ...
    <p>
      {`Bobril eco-system contains globalization package `}
      <code>bobril-g11n</code>
      {` to deal with localizations and formatting. It has to be added as a dependency to project.`}
    </p>
    <pre>
      <code class="language-bash">{`npm i bobril-g11n --save`}</code>
    </pre>
...
`}</code>
    ...
    <p>
      {`This code imports and calls `}
      <code>initGlobalization</code>
      {` function. Its argument with interface `}
      <code>IG11NConfig</code>
      {` defines the default locale and optionally the function for getting the path to the localized files. `}
      <code>Bobril-build</code>
      {` is configured to generate the default localization file `}
      <em>{`en-US.js`}</em>
    ...
    <p>
      {`Initialization of the application by `}
      <code>b.routes</code>
      {` has to be called in a fulfillment callback of this promise.`}
    </p>
    <h3 id="locale-switching">{`Locale switching`}</h3>
    <p>
      {`The example code changes the locale by `}
      <code>setLocale</code>
      {` and renders the page with specific translations. To get the current locale it uses the function `}
      <code>getLocale</code>
      {`.`}
    </p>
...
`}</code>
    ...
    <p>
      <a href="./static-examples/translations/index.html">{`Preview example`}</a>
    </p>
    <h3 id="translations">{`Translations`}</h3>
    <p>
      {`Translated content is defined by using the `}
      <code>t</code>
      {` function:`}
    </p>

    <pre>
      <code class="language-tsx">{`t("Hello World!");`}</code>
    </pre>
    <p>
      <a href="./static-examples/translations/index.html">{`Preview example`}</a>
    </p>
    ...
    <p>
      {`Bobril-build takes all usages of `}
      <code>t(&quot;some string&quot;)</code>
      {` in code and replaces it by e.g. `}
      <code>t(125)</code>
      {` where `}
      <em>{`125`}</em>
      {` is the index of `}
      <code>&quot;some string&quot;</code>
      {` constant in the array of localized strings. This array is placed in every localization file and corresponds to the array in the generated `}
      <em>{`en-US.js.`}</em>
    </p>
...
bb b -u 1`}</code>
    ...
    <p>
      {`The first command creates a new translation file `}
      <em>{`translations/cs-CZ.json`}</em>
      {`. The second command adds the missing translations from the default generated `}
      <em>{`en-US.js`}</em>
      {` to `}
      <em>{`cs-CZ.json`}</em>
      {` translation definition. The content of the created `}
      <em>{`json`}</em>
      {` can be e.g.:`}
    </p>
    <pre>
      <code class="language-json">{`["cs-CZ", ["My name is {a}!", null, 1]]`}</code>
    </pre>
    <p>{`To add translations it can be modified to the following:`}</p>
    <pre>
      <code class="language-json">{`["cs-CZ", ["My name is {a}!", null, 1, "Jmenuji se {a}!"]]`}</code>
    </pre>
    ...
    <ol>
      ...
      <li>
        {`Translation help (third optional parameter of t function) - null =not used in `}
        <code>t</code>
        {` function`}
      </li>
    ...
    <pre>
      <code class="language-tsx">{`t("My name is {a}!", { a: "Tomas" });`}</code>
    </pre>
    <h4 id="ordinal">{`Ordinal`}</h4>
    <p>{`To set localized ordinal, use the selectordinal pattern:`}</p>
    <p>
      <em>{`You are in 2nd floor`}</em>
    </p>

    <pre>
      <code class="language-tsx">{`t("you are in {floor, selectordinal, =0{ground} one{#st} two{#nd} few{#rd} other{#th}} floor", { floor: 2 });`}</code>
    </pre>
    ...
    <p>
      <em>{`here are 2 floors`}</em>
    </p>

    <pre>
      <code class="language-tsx">{`t("here {floor, plural, =0{is no floor} =1{is # floor} other{are # floors}}", { floor: 2 });`}</code>
    </pre>
    ...
    <pre>
      <code class="language-tsx">{`t("famous {gender, select, female {woman} male {man} other {person}}", { gender: "female" });`}</code>
    </pre>
    ...
    <pre>
      <code class="language-tsx">{`f("{arg, number}", { arg: 1.234 });`}</code>
    </pre>
    <p>
      <em>{`1.2340 in en`}</em>
    </p>
    ...
    <pre>
      <code class="language-tsx">{`f("{arg, number, custom, format:{0.0000}}", { arg: 1.234 });`}</code>
    </pre>
    ...
    <p>
      <em>{`Jan 2, 2000 12:00 AM - in en`}</em>
    </p>

    <pre>
      <code class="language-tsx">{`f("{a, date, lll}", { a: new Date(2000, 0, 2) });`}</code>
    </pre>
    <p>
      <em>{`02 01 - in en`}</em>
    </p>
    ...
    <pre>
      <code class="language-tsx">{`f("{a, date, custom, format:{DD MM}}", { a: new Date(2000, 0, 2) });`}</code>
    </pre>
    <p>
      <em>{`Sun - in en`}</em>
    </p>
    ...
    <pre>
      <code class="language-tsx">{`f("{a, date, custom, format:{{myFormat}} }", { a: new Date(2000, 0, 2), myFormat: "ddd" });`}</code>
    </pre>
    <p>
      {`The specific format definitions can be found in the `}
      <a href="http://momentjs.com/docs/#/displaying/format/">{`Moment.js documentation.`}</a>
    </p>
    <p>{`It can also be defined in a calendar format:`}</p>
    <p>
      <em>{`Tomorrow at 4:27 PM - in en`}</em>
    </p>

    <pre>
      <code class="language-tsx">{`f("{a, time, calendar}", { a: Date.now() + 24 * 60 * 60 * 1000 });`}</code>
    </pre>
    <p>{`or as a relative from now:`}</p>
    <p>
      <em>{`2 minutes ago - in en`}</em>
    </p>

    <pre>
      <code class="language-tsx">{`f("{a, time, relative}", { a: Date.now() - 100000 });`}</code>
    </pre>
    <h3 id="just-formatting">{`Just Formatting`}</h3>
    <p>
      {`If you only want to do the formatting of a text without a translation, just replace the `}
      <code>t</code>
      {` function by the `}
      <code>f</code>
      {` function.`}
    ...
    <p>
      {`Bobril has two ways of defining styles, `}
      <strong>{`inline styles`}</strong>
      {` and `}
      <strong>{`styles definitions`}</strong>
    </p>
...
export class CssBaseStyling extends b.Component<IStyledComponentData> {
    ...
    <ul>
      <li>
        {`To create CSS subclass use `}
        <code>b.styleDefEx</code>
        {`. First parameter is original class and the second is subclass style.`}
      </li>
...
export class CssExtendingStyling extends b.Component<IStyledComponentData> {
    ...
    <ul>
      <li>
        {`CSS selectors can be used in Bobril as well. Just use second optional parameter in `}
        <code>b.styleDef</code>
        {`.`}
      </li>
    </ul>
...
export class CssSelectorsStyling extends b.Component<IStyledComponentData> {
    ...
    <ul>
      <li>
        {`To define media query use `}
        <code>b.mediaQueryDef</code>
        {`, first parameter is media query and second affected styles with styling changes.`}
      </li>
      <li>
        {`Build-in builder `}
        <code>b.createMediaQuery</code>
        {` can be used to write media query.`}
      </li>
    </ul>
...
export class MediaQueriesStyling extends b.Component<IStyledComponentData> {
    ...
    <p>
      <a href="./static-examples/styling/index.html">{`Preview example`}</a>
    </p>
...
`}</code>
    ...
    <p>
      {`First you need to build your project with enabled sourcemaps. Because only new bundler support generating them it needs to be enabled with `}
      <code>newBundler</code>
      {` parameter. It is good to override sourceRoot in generated sourcemap to be just `}
      <code>.</code>
      {`, because it will remove one nesting from result.`}
    </p>
    <pre>
      <code class="language-bash">{`bb build --newBundler true --sourcemap yes --sourceRoot .`}</code>
    </pre>
    <p>{`or shorter`}</p>
    <pre>
      <code class="language-bash">{`bb b -x 1 -g yes --sourceRoot .`}</code>
    </pre>
    <p>
      {`Second and last step is to use `}
      <a href="https://github.com/danvk/source-map-explorer">{`source-map-explorer`}</a>
      {` on generated bundle. By default it will be in `}
      <code>dist</code>
      {` subdirectory and will be named `}
      <code>a.js</code>
      {`.`}
    </p>
    <pre>
      <code class="language-bash">{`npx source-map-explorer a.js`}</code>
    </pre>
  </>
);
